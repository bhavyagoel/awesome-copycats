...
awful.key({modkey1, "Shift"}, "Print", function()
    awful.util.spawn("scrot 'ScreenShot-%Y-%m-%d-%s.jpg' -e 'mv $f $$(xdg-user-dir PICTURES)'")
end, {
    description = "Scrot",
    group = "screenshots"
}), awful.key({"Shift"}, "Print", function()
    awful.util.spawn("scrot -s 'ScreenShot-%Y-%m-%d-%s.jpg' -e 'mv $f $$(xdg-user-dir PICTURES)'")
end, {
    description = "Window Select ",
    group = "screenshots"
}),
    -- awful.key({altkey }, "Print", function () awful.util.spawn("scrot -s /tmp/screenshot-$(date +%F_%T).png -e 'xclip -selection c -t image/png < $f'") end,
